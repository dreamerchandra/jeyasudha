...
function createBill (transaction, billingData) {
  ...
  transaction.set(newBillingRef, { ...billingData, createdAt: firebase.firestore.FieldValue.serverTimestamp() })
  return newBillingRef.id;
}

function createLedger (transaction, ledgerData) {
  const newLedgerRef = ref().ledger.doc();
  transaction.set(newLedgerRef, { ledgerData, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
}


function _updateBillingData ({
  userData,
  billingData,
  ledgerData,
  userId = '',
