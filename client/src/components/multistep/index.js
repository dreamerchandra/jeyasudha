...
export default function MultiStep(props) {
    ...
    show && (
      <div>
        <button
          ...
          onClick={previous}
          className="paper step-nav-controller"
        >
          Prev
        </button>
  ...
  return (
    <div onKeyDown={handleKeyDown} className="step-wrapper">
      <Ol>{renderSteps()}</Ol>
      {props.steps.map((step, index) => {
        const className =
          index === compState ? activeComponentClassName : inactiveComponentClassName
